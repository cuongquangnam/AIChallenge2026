...
class QARetrievalResult:
    ...
    video_score: float
    candidates: list[QACandidate]
    videos: list[QAVideoBucket] = field(default_factory=list)
...
class QACandidateRetriever:
    ...
    def retrieve(self, queries: list[str], *, top_videos: int = 1) -> QARetrievalResult:
        ...
        selected = ranked_videos[: max(top_videos, 1)]
        buckets: list[QAVideoBucket] = []
        for video_id, video_score in selected:
            candidates = self._frame_candidates(rankings, video_id=video_id)
            if candidates:
                buckets.append(
                    QAVideoBucket(
                        video_id=video_id,
        ...
        if not buckets:
            raise NoQACandidatesError("No frame candidates were found for any video")

        top = buckets[0]
        return QARetrievalResult(
            video_id=top.video_id,
            video_score=top.video_score,
            candidates=top.candidates,
            videos=buckets,
        )

    def _frame_candidates(
        self, rankings: list[list[SearchHit]], *, video_id: str
    ) -> list[QACandidate]:
